...
## エージェント共通運用（Claude / Codex）

Claude と Codex は同じ開発フローで作業する。

### 重要ルール

- linter / hook を絶対にバイパスしない（`--no-verify` / `--no-check` 禁止）
- 大きな変更に着手する前は設計方針を確認する
- feature / fix / chore ブランチで作業し、`main` への直接 push はしない
...
ユーザーが commit・push・PR 作成・レビュー・マージを依頼した場合は、追加確認なしで自律的に実行する。

- 適切な区切りでコミットし、作業ブランチを push する
- PR を作成し、CI / checks とレビュー観点を確認する
- 問題がなければ PR をマージする
- マージ後は必要に応じて次の作業ブランチを切る
...
- 機密情報がハードコードされていない
- `pnpm typecheck` / `pnpm lint` / 関連テストがパスする
